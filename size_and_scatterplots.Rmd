...
comparative by nature. Such work allows us to inform design guidelines as
well as human perception, in addition to affording us a more objective method of comparing
people's judgements beyond semantic labels such as 'weak' or 'strong' correlation as response measures.
...
point position more strongly than point contrast can \cite{hong_2021}, and can 
...
\cite{alais_2004}, and if this is driving the reduction in bias we saw in our previous work \cite{strain_2023},
...
of people's perceptions of correlations than previous work using fewer values (**maybe ref this**).
Previous work, including our own, used only positive *r* values. As we are building on this
here, we do the same. Section \autoref{future-work} discusses the testing of negative
...
in \cite{strain_2023}.In our fourth condition, which we refer to as *standard size*,
...
here when designing scatterplots optimized for correlation perception. Simultaneously
we acknowledge that for other tasks, such as cluster separation or numerosity perception,
the use of the manipulation described here would be a hindrance ??
