...
title: "Facilitating More Accurate Correlation Perception in Scatterplots"
editor_options: 
  markdown: 
    wrap: 72
...
The effect we observe here is much stronger, both with regards to effect size
...
followed by the visual threshold testing described above and the screen scaling task described in \autoref{dot-pitch-and-crowdsourced-experiments}.
...
# use geom_bracket() to add brackets for effects sizes between conditions
...
# use comparisonm function to create null model
...
# create table for effects sizes
...
knitr::kable(effects_sizes, digits = c(0,2), booktabs = TRUE, caption = "The effects sizes of the point size decay conditions when compared to the standard size condition.")
```
...
# chunk not included in final manuscript 
...
# plots r estimations errors against objective r value, separately for each condition
...
While \autoref{visual-threshold-testing}
...
objective *r* value itself. There is also the potential for manipulations
