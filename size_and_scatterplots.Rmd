...
Ppoint contrast is not the only available visual feature that might be used
...
from 150 participants was included in the analysis (`r printnum(gender$M, digits = 2)`% male, `r printnum(gender$F, digits = 2)`% female, and `r printnum(gender$NB, digits = 2)`% non-binary). Mean age of participants was `r printnum(age$mean)`
...
```{r dot-plot, echo=FALSE, fig.show='hold', fig.cap="Mean error in correlation estimates across the four size manipulation conditions, with 95\\% confidence intervals shown.", out.width="100%", crop = TRUE}
...
Table \ref{tab:effects-df}. The largest effect size we found was 0.61 when comparing
...
```{r changes-with-r-size, echo=FALSE, fig.align='center', fig.show='hold', fig.cap="hello", crop = TRUE, fig.env="figure*",crop = TRUE, out.width= "100%"}
...
As discussed previously, in the present study we employed a method fo obtaining
...
measurement as a fixed effect. Comparing this to the experimental model
