...
In our fourth condition, which we refer to as \emph{standard size}, point size was uniformly
...
and another including graph literacy as a fixed effect (\(\chi^2\)(1) 0.45, \emph{p} = .502), suggesting the effect
...
no significant effect of dot pitch (\(\chi^2\)(1) = 4.44, \emph{p} = .035)
...
= 1.28, \emph{p} = 0.26), suggesting that training having more recently viewed the example plots
...
\item
  We will investigate the effect of manipulating both point contrast and point
  size. Will these effects be additive, and to what degree?
\item
  We will introduce a parameter to control the strength of the manipulation
  according to the objective \emph{r} value in order to fine tune the effects we have found.
\end{enumerate}
