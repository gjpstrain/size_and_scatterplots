...
in our previous study \cite{strain_2023}. In our fourth condition,
which we henceforth refer to as *standard size*,
...
participants' errors being lower 
...
= `r printnum(model_dot_pitch_comparison.Chisq)`, *p* `r printp(model_dot_pitch_comparison.p, add_equals = TRUE)`). There was no
...
Lower salience and higher uncertainty exterior points have successfully reduced the perceived width 
of this probability distribution, which has resulted in more accurate estimates 
of correlation. Simultaneously we acknowledge that this is not the only candidate 
mechanism; we would expect similar results to emerge if a feature-based attentional
...
very similar to that in the standard size decay condition. Despite the size channel
...
other chart types, the use of the manipulation described here may in fact be a hindrance, and in scenarios
...
estimation, and we will introduce a parameter to control the strength of this 
