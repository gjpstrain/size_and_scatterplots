...
in those outer areas. We tested linear and non-linear, and inverted non-linear functions relating point contrast to
...
In this paper we utilize the same functions applied to point-size and demonstrate that the use of 
a non-linear decay function can be employed to correct for a systematic underestimation of correlation by 
...
of correlation (\cite{strain_2023}[^1]). Previous literature has found evidence for a pronounced underestimation in judgements
of correlation in positively correlated scatterplots, especially between 0.2 < *r* < 0.6. This
...
[^1]: Contains a full review of the history and contemporary issues regarding this work.

## Point Size

Contrast adjustments have been used extensively to solve issues of overplotting and clutter
in scatterplots \cite{matejka_2015, bertini_2004}. Common sense dictates that 
scatterplots visualizing larger datasets inherently require their points to be
smaller to prevent obfuscation of the data, and point size changes (e.g bubble charts)
...
From the wider literature there is evidence that larger points are more salient \cite{healey_2012}, can bias judgements of 
...
\cite{alais_2004}, and if this uncertainty is driving the reduction in bias we saw 
in our previous work \cite{strain_2023}, we would expect a similar effect when
...
of Manchester's Computer Science Departmental Panel (Ref: 2022-14660-24397). Hypotheses and analysis plans were 
pre-registered with the OSF (https://osf.io/k4gd8).

## Stimuli

45 *r* uniformly distributed *r* values were generated between 0.2 and 0.99. 
...
identical. This range of *r* values was chosen as there is evidence from the literature 
...
Previous work, including our own, used only positive *r* values, and as we are building on this
here, we do the same. Each scatterplot had a 1:1 aspect ratio,
was generated as a 1000 x 1000 pixel .png image, and was
...
*b* may exist, but extensive testing of this is outside the scope of the current study.
We additionally adjusted the residual values using a scaling factor of 4 and a
constant of 0.2 to achieve a minimum on-screen point size of 12 pixels, which 
is consistent with the point size on a 1920 x 1080 monitor for both experiments
in our previous study \cite{strain_2023}. In our fourth condition, which we henceforth refer to as *standard size*,
...
contexts and on a range of apparatus, we included point visibility testing prior
...
followed by the point visibility testing described above and the screen scaling task
described in \autoref{dot-pitch-and-crowdsourced-experiments}.
...
A likelihood ratio test revealed that the
...
*p* `r printp(size_model.p, add_equals = TRUE)`).  The effect here is driven by participants' errors being lower 
...
with standard size than for plots with inverted non-linear decay. Figure
\ref{fig:differences-plot} shows how participants' mean errors in correlation
...
was performed with the **emmeans** package (version 1.8.7 \cite{emmeans}), and
are shown in Table \ref{tab:contrasts-table}. The **EMAtools** (version
0.1.4 \cite{ematools}) package was used to calculate effect sizes in Cohen's d,
...
and another including graph literacy as a fixed effect 
...
While participants performed well on a point , there may be some other facet of using a larger or smaller
...
a significant effect of dot pitch ($\chi^2$(`r in_paren(model_dot_pitch_comparison.df)`) = `r printnum(model_dot_pitch_comparison.Chisq)`, *p* `r printp(model_dot_pitch_comparison.p, add_equals = TRUE)`). There was no
...
size is a stronger channel for the manipulation of perceived correlation in
...
conditions became smaller as the objective *r* value increased, participants' 
...
and linear size decay conditions (see examples in the item preparation folder 
in the repository linked above). It may be that this
...
Ultimately we aim to provide tools that can be used to design visualizations more suited for the tasks
...
from drawing concrete psychophysical conclusions,
...
and may even be making perception worse, at very low and high values of *r*. We will therefore
investigate the effect of manipulating *both* point size and contrast on correlation estimation, and we will introduce a
parameter to control the strength of this family of manipulations according to the 
objective *r* value itself.  More qualitative work is needed to both address any misinterpretation
