...
in those outer areas. We tested linear and non-linear functions relating point contrast to
residual magnitude, together with an inverted non-linear function condition. As
anticipated, we found that the non-linear function produced
...
In this paper we present a single experiment study in which we demonstrate that the use of 
...
[^1]: Contains a full review of this work.
...
have been used to represent trivariate data. There has been little testing of the 
...
We additionally adjusted the residual values using a scaling factor of 4 and a constant of 0.2 to achieve a minimum on-screen point size of 12 pixels, which is consistent with the point size on a 1920 x 1080 monitor for both experiments
...
# extract perceptage correct on visual thresholds
...
```{r changes-with-r-size, echo=FALSE, fig.show='hold', fig.cap="Participants' mean errors in \\textit{r} estimates plotted against the objective \\textit{r} value separately for each size decay condition. The dotted horizontal line represents accurate estimation.", fig.env="figure*", out.width= "100%"}
...
\ref{fig:changes-with-r-size} shows how participants' mean errors in correlation
...
the non-linear size decay and standard size decay conditions.
...
for within a particular experiment. The stimuli as *r* approaches 1 in the current
